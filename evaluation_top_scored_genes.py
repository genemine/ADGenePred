...
def seq_validation(genelist,num,times):
    ...
    p_val=p_value(random_score,observed)
...
def ppi_validation(ad_gene,num,genelist,times):
    ...
    gene=genelist[:num]
    observed_score,observed_num=stats_ppi(gene,stats)
    random_score=[]
    random_num=[]
    for i in range(times):
        gene=genelist[random.sample(range(len(genelist)),num)]
...
def mirna_validation(ad_gene,num,genelist,times):
    ...
    observed=len(mirna_top.intersection(mirna_ad))
    p_val=p_value(random_num,observed)
    randomx=np.mean(random_num)
    f=open('validation_result/mirna_evaluation.txt','w')
    write_file(p_val,observed,randomx,f)
    return p_val,observed,random_num
...
seq_pval,seq_observed,seq_random_score=seq_validation(genelist,num,times)

coexp_pval,coexp_observed,coexp_random_score=coexpr_validation(num,genelist,times)

ppi_validation(ad_gene,num,genelist,times)

miRNA_pval,miRNA_observed,miRNA_random_num=mirna_validation(ad_gene,num,genelist,times)      
